...
\textcolor{red}{For each user, we compute } 
%For computing 
the preference $pref_c$ and the confidence $conf_c$ of each class $c$, \textcolor{red}{according to}
%we use 
Eq. (\ref{eq:preference}) and Eq. (\ref{eq:confidence}), respectively; 
where $ancestors(c)$ is the set of ancestors of the ontology class $c$, $pref_p$ is the user's preference of the class $p$, $conf_p$ is the confidence \textcolor{red}{about the user's preference for}
%of the 
class $p$, and $\alpha$ is the \textit{decrease rate} \textcolor{red}{indicating}
%that will tell 
