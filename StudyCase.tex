...
Fuseki server as the triplestore, Apache Jena for traversing the ontology and connecting to the triplestore, and a MySQL database for storing additional information of the nodes and items. \textcolor{red}{To have initial data, we requested real users through questionnaires, whose information was used to  create simulated users and scenarios to test with.}
%Some initial data were obtained from real users through questionnaires and we simulate several scenarios  and users to test with.

%In this section, 
\textcolor{red}{With this version of RECESO,} we are particularly testing the User Interest Model Module, the Recommendation Module, and the Activation Propagation \textcolor{red}{algorithm} from the Context Module, while the Data Gathering Module and the context gathering (Location and Context Factors) are simulated.
...
\begin{itemize}
    ...
    \item 5 means you do not care if the context is met or not.
    \item 10 means that if the context is met, you would go to the place/event.
\end{itemize}

Then, Eq. (\ref{eq:avg}) is applied to each context factor, hence average them and transforming them to be on range $[0,2]$ by dividing by $5$. \textcolor{red}{Este promedio sería por context factor por clase? Hay que aclararlo.} 

\textcolor{red}{CREo que podemos decir que los valores eran 0 1 o 2 en la encuesta y no tenemos que decir que se dividió entre 5, ni poner la fórmula del promedio. Tan sólo decir que se promedió. }
...
%the \textit{Elbow Method} (see Figure \ref{fig:elbow}) was used to choose $4$ as the number of clusters, since increasing this number does not make improvements worth the cost using our small dataset. In future work, a larger dataset should be used.
...
For each \textcolor{red}{tourist $T_i$,}
%centroid of the clusters mentioned previously, 
we simulate two visits to Niza, Lyon, and Paris for each possible context using each possible value for each context factor \textcolor{blue}{Se podría poner esto en una tablita para verlo más fácilmente, quizás por $T_i$?}\textcolor{red}{O ilustrar con un Ti, las combinaciones y las visitas. DISCUTIR}. The system returns a set of not more than $5$ recommended places inside a radius of $8$ kilometers for each visit; this set is going to be called the \textit{recommendation}. Let $R$ be a recommendation \textcolor{red}{R no sería the \textit{recommendation} set ?} for a specific user on a specific location \textcolor{red}{(i.e., Niza, Lyon, or Paris) (???)} and a specific context \textcolor{red}{qué es un contexto específico: una combinación de factores?}, we define \textcolor{red}{some metrics to evaluate how "good" is a \textit{recommendation} $R$, as follows:}
%the following recommendation metrics:

\begin{itemize}
    \item Average of preference \textcolor{red}{(Eq. (XXX))}, which denotes how near to the user's preferences are the recommended POI.
    \begin{equation}
    pref_{R} = \frac{ \displaystyle \sum_{p \in R}{pref_p} }{| R |}
\end{equation}

\item \textcolor{red}{y así explicar cada métrica con su fórmula. Otra cosa, todas las métricas deberían tener $u$ o ninguna, pues se sobre entiende que estas métricas son por usuario.}
\end{itemize}
...
Finally, the system returns the average of each recommendation metric. \textcolor{red}{Pero ya todas las métricas son promedios, luego con qué promedia?}


\textcolor{red}{esto de novelty, no debería ir antes de ese "Finally," del párrafo anterior?}
...
\textcolor{red}{To calculate the score for each POI $p$, we need some user-defined parameters, that represent maximum distance ($maxdist$) from the user to the PIO and the weighs ($w_i$) for each term in Eq. (\ref{eq:score})   (see Section \ref{section:score}). We designed two scenarios with different values of these configuration parameters, as we explain in the following.}  

%We use different coefficients for the score (see Section \ref{section:score})  
%, and since we said $maxdist$ is $8$ for these experiments, we are ignoring it as argument. 

\subsubsection{First configuration} 
\label{section:experiment-1}
We consider $maxdist =8 Km$, $w_1 = \frac{1}{4} + \frac{1}{9}$ for preference, $w_2 = \frac{1}{4} + \frac{1}{9}$ for activation, $w_3 = \frac{1}{4}$ for aging, and $w_4 = \frac{1}{4} - \frac{2}{9}$ for distance from user, resulting on 
%equation 
Eq. (\ref{eq:score-1}). \textcolor{red}{In this configuration, we give}
%and giving 
...
\textcolor{red}{Por qué los $w_i$ son así?, no podemos decir simplemente, 0.35 , 0.35, 0.25 y 0.05 ?}
