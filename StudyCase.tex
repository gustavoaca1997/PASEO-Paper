...
\textcolor{red}{Decir que en particular estamos evaluando el modulo de recomendacion y de contexto?, cuál modulo no está implementado? }
...
\begin{enumerate}
    \item To determine the relevance for a user that a {\it context} factor has to decide to go to a {\it place/event}. The list of context factors included \textcolor{red}{COLOCAR CUALES}; and the place/event were related to the high level classes listed in the previous section. The question was "how much  'context factor' influences your decision to go to a 'place/event' of the specified type (these types are not exclusive)"  \textcolor{red}{NO eniendo muy bien la pregunta. Discutir}
    
people should answer a real value between $0$ and $10$, where: 
\begin{itemize}
...
\end{itemize}
\textcolor{red}{Podemos cambiar a valor entero?}

Then, Eq. (\ref{eq:avg}) is applied to each feature or column, hence average them and transforming them to be on range $[0,2]$.
\begin{equation}
\label{eq:avg}
AVG(column)/5
\end{equation}

\textcolor{red}{Por qué entre 5? quienes son las columnas?}
...
\end{enumerate}
    ...
    Both for relevance and preferences, we got around 100 answers from people of different ages, professions, and countries.  
    
\textcolor{red}{Explicar que con esta información se generaron usuarios sintéticos en diferentes escenarios de contextos y comotamientos  }
...
\textcolor{red}{In a future version of our system,  this information will be gathered ........}


\subsubsection{Simulated scenarios}
To evaluate the system, it was decided to test it with simulated scenarios (simulated contexts and simulated users). A set of centroids of a set of clusters were chosen as the set of simulated users \textcolor{red}{De cual parte del cuestionario?}. The \textit{K-Means} algorithm was used to get the clusters and the \textit{Elbow Method} (see Figure \ref{fig:elbow}) was used to choose $4$ as the number of clusters. \textcolor{red}{Se puede explicar por qué 4?}
...
\textcolor{red}{Y con las relevancias qué se hizo? }
...
\textcolor{red}{Me parece que todas estas métricas y el proceso de recomendación  se deben explicar en la propuesta, en el módulo de recomendaión (el nro de recomendaciones es un parametro de configuracion). Aquí entonces solo se hablaria de que se consideranron los siguientes parámtros : nro recomem= 5, maxdist=8km asumiendo que el medio de transporte es auto o bici? Mejor hablar de todo la configuración, donde colocas las configurcaiones}
