...
\textcolor{red}{The data gathered with questionnaires were grouped into four cluster, with the \textit{Elbow Method} (increasing the number of clusters  does not make improvements worth the cost using our small dataset (see Figure \ref{fig:elbow})).}
...
\textcolor{red}{For each tourist $T_i$, we simulate visits to Paris, Niza, and Lyon. For each combination of context factors ($24$ combinations),  each $T_i$ goes twice to each site; thus $48$ visits for each place (see Figure \ref{fig:experiments}).
%values 
